...
export function Onboarding({ initial, studio, projects, projectId, initialDate }: { initial: Profile; studio: StudioState; projects: Project[]; projectId: string; initialDate: string }) {
  ...
  return <div className="grid gap-10 lg:grid-cols-[220px_1fr]">
    ...
    <div className="studio-panel"><DraftNotice conflict={profile.conflict !== null} restore={profile.restore} />
      {step < 6 && <fieldset disabled={busy || uploading}><ProfileFields key={step} profile={profile.value} onChange={value => { profile.update(value); setError(""); setStatus("Saving…"); }} section={step} onBusy={setUploading} /></fieldset>}
      {step === 6 && <><p className="mb-6 text-sm text-moss">{savedProjects.length ? `You already have ${savedProjects.length} projects. Add another here, or skip to the preview.` : "Start with one project. You can add the rest later."}</p><div className="mb-6 flex gap-2">{["Details", "Images", "Review"].map((name, i) => <button key={name} type="button" disabled={busy || uploading} className={projectStep === i ? "section-tab active" : "section-tab"} onClick={() => { setProjectStep(i); setError(""); }}>{name}</button>)}</div><fieldset disabled={busy || uploading}><ProjectFields key={projectStep} project={project.value} onChange={value => { project.update(value); setError(""); setProjectAdded(false); setStatus("Saving draft…"); }} section={projectStep} onBusy={setUploading} /></fieldset><div className="mt-6 flex justify-end">{projectStep < 2 ? <button type="button" className="btn-secondary" disabled={uploading} onClick={() => { setProjectStep(projectStep + 1); setError(""); }}>Next: {projectStep === 0 ? "Images" : "Review"} →</button> : <button type="button" className="btn-primary" disabled={busy || uploading} onClick={() => void saveProject()}>Save first project</button>}</div></>}
