...
import { AttentionIcon, FeedbackScope, useDraftFeedback } from "./DraftFeedback";
...
export function Field({ label, value, onChange, required, multiline, hint, type = "text", maxLength = 20000 }: { label: string; value: string; onChange: (value: string) => void; required?: boolean; multiline?: boolean; hint?: string; type?: string; maxLength?: number }) {
  const id = useId(), feedback = useDraftFeedback(label), description = [hint ? `${id}-hint` : "", feedback.issue ? feedback.errorId : ""].filter(Boolean).join(" ") || undefined;
  return <div className="studio-field" data-feedback-key={feedback.key} onBlur={feedback.touch}><label htmlFor={id}>{label} {feedback.issue && <AttentionIcon />}{required && <span aria-hidden="true"> *</span>}</label>{hint && <small id={`${id}-hint`}>{hint}</small>}
    {multiline ? <textarea id={id} aria-label={label} aria-invalid={!!feedback.issue || undefined} aria-describedby={description} className="admin-input min-h-36" value={value} required={required} maxLength={maxLength} onChange={e => onChange(e.target.value)} /> : <input id={id} aria-label={label} aria-invalid={!!feedback.issue || undefined} aria-describedby={description} className="admin-input" type={type} value={value} required={required} maxLength={maxLength} onChange={e => onChange(e.target.value)} />}{feedback.issue && <small id={feedback.errorId} className="!text-[#8e302b]">{feedback.issue.message}</small>}
  </div>;
}
...
function ImagePicker({ label, value, onChange, onBusy, required = false }: { label: string; value: string; onChange: (value: string) => void; onBusy?: (busy: boolean) => void; required?: boolean }) {
  ...
  return <div className="image-picker" data-feedback-key={feedback.key} onBlur={feedback.touch}>
    ...
    <div className="min-w-0 flex-1">{local ? <p className="text-sm font-bold">{label} {feedback.issue && <AttentionIcon />}{required ? " *" : ""}</p> : <Field label={label} value={value} required={required} hint="Upload and crop a photo, or use an existing /images/ path." onChange={value => { setError(""); onChange(value); }} />}
      <label className="mt-3 block text-sm"><span className="sr-only">Upload {label.toLowerCase()}</span><input className="w-full text-xs file:mr-3 file:rounded-full file:border file:border-line file:bg-paper file:px-3 file:py-2 file:font-semibold" type="file" aria-invalid={!!feedback.issue || undefined} aria-describedby={feedback.issue ? feedback.errorId : undefined} accept="image/png,image/jpeg,image/webp" disabled={busy} onChange={e => { choose(e.target.files?.[0]); e.target.value = ""; }} /></label>
      ...
      {feedback.issue && <p id={feedback.errorId} className="mt-2 text-sm text-[#8e302b]">{feedback.issue.message}</p>}{error && <p className="form-error mt-2" role="alert">{error}</p>}
...
export function ProfileFields({ profile, onChange, section, onBusy }: { profile: Profile; onChange: (value: Profile) => void; section: number; onBusy?: (busy: boolean) => void }) {
  ...
  if (section === 2) return <Field label="Biography" required value={profile.biography} multiline hint="Shown in full on Home and About. Markdown is supported: **bold**, *italic*, lists, and links." onChange={v => change("biography", v)} />;
  if (section === 4) return <SoftwarePicker value={profile.tools} onChange={v => change("tools", v)} />;
  if (section === 3) return <div className="space-y-5">{!profile.education.length && <p className="empty-note">Where did you learn your craft? Add a degree, course, or program.</p>}{profile.education.map((entry, index) => <FeedbackScope.Provider value={`education.${index}`} key={index}><fieldset className="entry-card"><legend className="px-2 text-sm font-bold">Education {index + 1}</legend><div className="grid gap-4 sm:grid-cols-2">{(["institution", "degree", "field", "startYear", "endYear"] as const).map(key => key === "startYear" || key === "endYear" ? <DateField key={key} label={key === "startYear" ? "Start date" : "End date"} value={entry[key]} allowPresent={key === "endYear"} onChange={value => change("education", profile.education.map((item, i) => i === index ? { ...item, [key]: value } : item))} /> : <Field key={key} label={{ institution: "Institution", degree: "Degree", field: "Field of study", startYear: "Start year", endYear: "End year" }[key]} required={key === "institution"} value={entry[key]} onChange={value => change("education", profile.education.map((item, i) => i === index ? { ...item, [key]: value } : item))} />)}</div><div className="mt-4"><Field label="Education description" multiline value={entry.description || ""} onChange={value => change("education", profile.education.map((item, i) => i === index ? { ...item, description: value } : item))} hint="Optional. Describe your focus, achievements, or relevant coursework." /></div><button className="btn-text mt-4" type="button" onClick={() => change("education", profile.education.filter((_, i) => i !== index))}>Remove education {index + 1}</button></fieldset></FeedbackScope.Provider>)}<button className="btn-secondary" type="button" onClick={() => change("education", [...profile.education, { institution: "", degree: "", field: "", startYear: todayISO(), endYear: todayISO() }])}>+ Add education</button></div>;
  return <div className="space-y-5">{!profile.jobs.length && <p className="empty-note">Add the roles, collaborations, and independent work that shaped your practice.</p>}{profile.jobs.map((entry, index) => <FeedbackScope.Provider value={"job." + index} key={index}><fieldset className="entry-card"><legend className="px-2 text-sm font-bold">Experience {index + 1}</legend><div className="grid gap-4 sm:grid-cols-2">{(["company", "position", "startDate", "endDate"] as const).map(key => key === "startDate" || key === "endDate" ? <DateField key={key} label={key === "startDate" ? "Start date" : "End date"} value={entry[key]} allowPresent={key === "endDate"} onChange={value => change("jobs", profile.jobs.map((item, i) => i === index ? { ...item, [key]: value } : item))} /> : <Field key={key} label={{ company: "Company", position: "Position", startDate: "Start date", endDate: "End date" }[key]} required={key === "company" || key === "position"} value={entry[key]} onChange={value => change("jobs", profile.jobs.map((item, i) => i === index ? { ...item, [key]: value } : item))} />)}</div><div className="mt-4"><Field label="Role description" value={entry.description} multiline onChange={value => change("jobs", profile.jobs.map((item, i) => i === index ? { ...item, description: value } : item))} /></div><button className="btn-text mt-4" type="button" onClick={() => change("jobs", profile.jobs.filter((_, i) => i !== index))}>Remove experience {index + 1}</button></fieldset></FeedbackScope.Provider>)}<button className="btn-secondary" type="button" onClick={() => change("jobs", [...profile.jobs, { company: "", position: "", description: "", startDate: todayISO(), endDate: todayISO() }])}>+ Add experience</button></div>;
}
