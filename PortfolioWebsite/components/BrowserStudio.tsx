...
export function BrowserStudio() {
    ...
    void (async () => {
      ...
      if (auth && active) stop = onAuthStateChanged(auth, user => {
        ...
        if (accountUid.current !== uid) {
          ...
          if (!uid && key.current !== "guest") void switchDraft("guest").catch(e => setError(e instanceof Error ? e.message : "Could not open your guest draft."));
        }
      });
  ...
  <main id="main-content" className="browser-studio mx-auto max-w-6xl px-6 pb-20 md:px-10">{welcome && <p className="notice mb-5" role="status">{welcome}</p>}<div className="mb-5 border-y border-line py-3"><p className="text-sm leading-6 text-moss">Signed-in drafts save privately to your account. Publish when you want visitors to see your changes. Guest drafts stay on this device.</p></div>
