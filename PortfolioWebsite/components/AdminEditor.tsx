...
export function DeleteProject({ id, title }: { id: string; title: string }) {
  ...
  return <div>{confirm ? <div className="mt-3 rounded-xl border border-line bg-paper p-4"><p className="mb-3 text-sm">Delete “{title}”? This removes the project from your next deployment. Image files are kept.</p><div className="flex gap-4"><button type="button" className="btn-danger" disabled={busy} onClick={() => void remove()}>{busy ? "Deleting…" : "Confirm delete"}</button><button type="button" className="btn-secondary" disabled={busy} onClick={() => setConfirm(false)}>Keep project</button></div></div> : <button type="button" className="btn-text" onClick={() => setConfirm(true)}>Delete</button>}{error && <p className="form-error" role="alert">{error}</p>}</div>;
}
