...
export function DeleteAccount({ user, disabled, remove, onBusy }: { user: User; disabled: boolean; remove: (uid: string) => Promise<void>; onBusy: (busy: boolean) => void }) {
  ...
  return <div className="border-t border-line pt-5"><h3 className="font-bold">Delete account</h3><p className="mt-2 text-sm leading-6 text-moss">Permanently remove your account, published website, uploaded images, and saved account draft, including its device copy.</p><button className="btn-danger mt-4" disabled={disabled} onClick={() => setOpen(true)}>Delete account</button>{open && <Confirmation user={user} remove={remove} onBusy={onBusy} close={() => setOpen(false)} />}</div>;
}
