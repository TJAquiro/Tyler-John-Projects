...
export type Account = { id: string; email: string; name: string; handle: string; salt: string; hash: string; createdAt: string };
...
export async function registerAccount(input: { name: string; email: string; password: string; handle: string; importExisting?: boolean }) {
  ...
  const account = { id: randomUUID(), name, email, handle, salt, hash, createdAt: new Date().toISOString() };
  initializePortfolio(account.id, name, Boolean(input.importExisting && accounts.length === 0));
