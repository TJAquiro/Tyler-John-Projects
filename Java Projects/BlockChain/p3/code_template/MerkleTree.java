...
public class MerkleTree
{
	...
	/**
	 * Returns a SinglyLinkedList of hashes needed to to make the root hash when combined with the provided Transaction.
     ...
     * @return boolean whether the hashToFind has been found.
     */
	private boolean extractProofRecursive(Node current, String hashToFind)
    {
		...
		if(current.getleft().data.equals(hashToFind))
    	{
    		extractProofreturn.add(current.getright().data);
    		return true;
    	}
    	else if(current.getright().data.equals(hashToFind))
    	{
