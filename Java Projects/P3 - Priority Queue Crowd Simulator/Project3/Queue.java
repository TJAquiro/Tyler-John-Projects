...
public class Queue {
	...
	public String toString()
	{
		...
		for(int x = 0;x < clientsInQueue.length; ++x)
		{
			...
			else
			{
				...
				if (clientsInQueue[x].getId() < 10)
				{
					addstring = addstring + "[" + "0" + (clientsInQueue[x]).getId() + "]";
				}
				else
				{
	...
	public String toString(boolean showID)
	{
		if(showID != true)
		{
			...
			if (clientBeingServed != null && clientBeingServed.getRequests() != null)
			{
			...
			//if currentClientAdder - clientBeingServed.getServiceTime() is a 1 digit number and a 0 in front
			if (currentClientAdder - clientBeingServed.getServiceTime() < 10)
			{
				return ("[Queue:" + (this.getIndex(QueueSystem.getQueues()) + 1) + "]" + "[" + "0" + (currentClientAdder - clientBeingServed.getServiceTime()) + "]" + "-----" + addstring);
			}
			else
				return ("[Queue:" + (this.getIndex(QueueSystem.getQueues()) + 1) + "]" + "[" + (currentClientAdder - clientBeingServed.getServiceTime()) + "]" + "-----" + addstring);
		}
		else
		{
