		...
		if(playerNum == playerNumToWin)
		{
				...
				if(game.isLegal(x))
				{
					if(simulateTurn(playerNum * -1, x, game) == playerNum * -1)
					{
						...
						{	
							...
							return new MultiOutput(x, depth, -50);
						}
					}
				}
			}
			...
			for(int x = 0;x < 7; ++x)
			{
				if(game.isLegal(x))
				{
					...
					if(x == 3 && bestEval <= result.getEval())
					{
						bestEval = result.getEval()+1;
						bestOptionCol = x;
					}
		...
		else
		{
			...
			for(int x = 0;x < 7; ++x)
			{
				if(game.isLegal(x))
				{
					...
					if(x == 3 && result.getEval() <= WorstEval)
					{
						WorstEval = result.getEval()-1;
						bestOptionCol = x;
					}
					else if(result.getEval() <= WorstEval)
					{
